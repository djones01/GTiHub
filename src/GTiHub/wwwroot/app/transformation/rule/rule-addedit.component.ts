...
export class RuleAddEditComponent implements OnInit, OnDestroy {
    ...
    constructor(private _dataService: DataService, private modalService: NgbModal, private sourceSelectService: SFieldSelectService, private targetSelectService: TFieldSelectService, private MapAddEditService: MapAddEditService) {
        this.rule = new Rule('', '', 'text', null, null);
    }
    ngOnInit(): void {
        this.ruleSubscription = this.MapAddEditService.getRule().subscribe((rule: Rule) => this.rule = rule);
        this.hasSelectedTargetFieldSubscription = this.targetSelectService.hasSelectedTargetField().subscribe(hasSelectedTargetField => this.hasSelectedTargetField = hasSelectedTargetField);
        this.getSelectedTargetFieldSubscription = this.targetSelectService.getSelectedTargetField().subscribe(selectedTargetField => this.selectedTargetField = selectedTargetField);
    }
    ngOnDestroy(): void {
